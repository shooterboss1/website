<?php
/**
 * Helper Script: Add Products to Men's Collection
 * This script helps you add sample men's products to your database
 */

require_once 'config.php';

echo "<h1>Add Men's Products to Database</h1>";
echo "<style>body{font-family:Arial;padding:40px;} .success{color:green;} .error{color:red;} .info{color:blue;}</style>";

// Sample men's products to add
$mensProducts = [
    [
...
if ($action === 'add') {
    echo "<h2>Adding Men's Products...</h2>";
    ...
    echo "<p><a href='men.php'>View Men's Collection →</a></p>";
    echo "<p><a href='add_men_products.php'>← Back</a></p>";
    
} elseif ($action === 'convert') {
    // Convert existing products to men's category
    echo "<h2>Converting Existing Products to Men's Category...</h2>";
    echo "<p class='info'>This will change existing products to men's category based on your selection.</p>";
    
    // Get current products
    $result = $conn->query("SELECT id, name, category FROM products WHERE category != 'men' ORDER BY id DESC LIMIT 10");
    
    echo "<form method='POST' action='?action=convert_confirm'>";
    echo "<h3>Select products to convert to Men's category:</h3>";
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<br><button type='submit' style='padding:10px 20px; background:#667eea; color:#fff; border:none; cursor:pointer; border-radius:4px;'>Convert Selected to Men's</button>";
    } else {
    ...
    foreach ($_POST['products'] as $productId) {
        $stmt = $conn->prepare("UPDATE products SET category = 'men' WHERE id = ?");
        $stmt->bind_param("i", $productId);
        
        if ($stmt->execute()) {
            $converted++;
            echo "<p class='success'>✅ Product ID $productId converted to men's category</p>";
        }
    }
    
    echo "<hr>";
    echo "<h3 class='success'>Converted $converted products to Men's category!</h3>";
    echo "<p><a href='men.php'>View Men's Collection →</a></p>";
    echo "<p><a href='add_men_products.php'>← Back</a></p>";
    
} elseif ($action === 'manual_add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = $_POST['name'];
    $price = $_POST['price'];
    $image = $_POST['image'];
    $rating = $_POST['rating'];
    ...
    $stmt = $conn->prepare("INSERT INTO products (name, price, category, image, rating, product_condition) VALUES (?, ?, 'men', ?, ?, ?)");
    $stmt->bind_param("sdsds", $name, $price, $image, $rating, $condition);
    
    if ($stmt->execute()) {
        echo "<h2 class='success'>✅ Successfully Added!</h2>";
        echo "<p><strong>$name</strong> has been added to the men's collection.</p>";
        echo "<p><a href='men.php'>View Men's Collection →</a></p>";
        echo "<p><a href='add_men_products.php'>← Add Another</a></p>";
    } else {
        echo "<h2 class='error'>❌ Error</h2>";
        echo "<p>Failed to add product: " . $conn->error . "</p>";
        echo "<p><a href='add_men_products.php'>← Try Again</a></p>";
    }
} else {
    // Show options
    ?>
    ...
    <div style="margin: 30px 0;">
        <h3>Option 1: Add Sample Men's Products</h3>
        <p>This will add 6 new men's products to your database.</p>
        <ul>
    ...
    <div style="margin: 30px 0;">
        <h3>Option 2: Convert Existing Products to Men's Category</h3>
        <p>Change existing products from other categories to men's category.</p>
    ...
    <div style="margin: 30px 0;">
        ...
        <p>Add your own custom men's product.</p>
        <form method='POST' action='?action=manual_add' style="max-width: 500px;">
            ...
            <div style="margin: 15px 0;">
                ...
                <input type="text" name="image" value="images/men-product.jpg" required style="width:100%; padding:8px; border:1px solid #ddd; border-radius:4px;">
                <small style="color:#666;">e.g., images/men-product.jpg</small>
            </div>
            ...
            <button type="submit" style="padding:12px 24px; background:#667eea; color:#fff; border:none; cursor:pointer; border-radius:4px; font-weight:bold;">
                Add Men's Product
            </button>
        </form>
    </div>
    ...
    <div style="background:#f0f8ff; padding:20px; border-radius:8px; border-left:4px solid #667eea;">
        ...
        <?php
        $menCount = $conn->query("SELECT COUNT(*) as count FROM products WHERE category = 'men'")->fetch_assoc();
        $totalCount = $conn->query("SELECT COUNT(*) as count FROM products")->fetch_assoc();
        
        echo "<p><strong>Men's Products:</strong> {$menCount['count']}</p>";
        echo "<p><strong>Total Products:</strong> {$totalCount['count']}</p>";
        
        if ($menCount['count'] == 0) {
            echo "<p class='error'><strong>⚠️ No men's products found!</strong> Use one of the options above to add some.</p>";
        } else {
            echo "<p class='success'><strong>✅ Men's collection has {$menCount['count']} products</strong></p>";
            echo "<p><a href='men.php' style='color:#667eea; font-weight:bold;'>View Men's Collection →</a></p>";
        }
        ?>
    </div>
